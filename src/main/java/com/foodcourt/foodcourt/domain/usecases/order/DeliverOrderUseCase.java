...
import static com.foodcourt.foodcourt.domain.constants.OrderErrorMessage.*;
...
public class DeliverOrderUseCase implements DeliverOrderPort {
	...
	public void execute(Long idOrder, String securePin, UserClaims userClaims) {
		...
		validateSecurePin(existingOrder.getSecurePin(), securePin);
	...
	private Order validateOrder(Long idOrder, Long idUser) {
		...
		if (!COMPLETED.equals(existingOrder.getStatus()))
			throw new InvalidOrderStatusException(ORDER_STATUS_MUST_BE_COMPLETED);
	...
	private void validateSecurePin(String existingPin, String providedPin) {
